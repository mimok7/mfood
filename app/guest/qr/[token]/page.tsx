...
async function fetchMenu(token: string): Promise<MenuItem[]> {
  const base = process.env.NEXT_PUBLIC_BASE_URL || ''
  const res = await fetch(`${base}/api/guest/menu?token=${encodeURIComponent(token)}`, { cache: 'no-store' })
  if (!res.ok) return []
  return res.json()
}

export default async function GuestQrPage({ params }: { params?: Promise<{ token: string }> }) {
  const resolvedParams = params ? await params : undefined
  const items = await fetchMenu(resolvedParams?.token ?? '')
  return (
